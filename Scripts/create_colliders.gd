...
func createCollider(length:float,angle:float,pos:Vector2):
	...
	if(not in_draw_mode and not has_skeleton):
		...
		for i in range(1,len(points_array)-1):
			...
			if(abs(slope_last_frame - slope) >= slope_collider_sensitivity):
				...
				createCollider(length,angle,col_position)
			...
			slope_last_frame = slope
		angle = atan(slope_last_frame)
		createCollider(length,angle,(points_array[-1] + last_detPos)/2)
		has_skeleton = true
