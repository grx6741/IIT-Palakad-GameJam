...
func setInstructions():
	...
	-> Hit Clear button before next step to regain ink
	-> Space Bar to instill life 
	...
	Stroke Mode -> JOINT_STROKE
		Connect two nearby nodes highlighted by PURPlE,
	...
	Stroke Mode -> ROTATION_STROKE_3PI
		Rotates on confirmation at fast velocity
	'''

func _ready():
	_textbox = $RichTextLabel
	_optionNode = get_node("../PlayerInterface/UI/StrokeOptions")
	setInstructions()
	print(_optionNode)
